...
for n = 1:length(NVec)
    ...
    for t = 1:length(TVec)
        ...
        Obj((n-1)*length(TVec)+t).FailureNumber = 0;
        Obj((n-1)*length(TVec)+t).Success_Rate = 1;
...
%         Obj((n-1)*length(TVec)+t).OptimizationTime_M4 = zeros(1,NS);
        ...
        for s = 1:NS
            ...
            t
            s
            Success_Rate = 1 - Obj((n-1)*length(TVec)+t).FailureNumber/s
...
%             fprintf('\nM4 finished! \n');
%             
%             if abs(max(demand_M1 - demand_M4)) > 10^(-2) || abs(max(generation_M1 - generation_M4)) > 10^(-2)
%                 Obj((n-1)*length(TVec)+t).FailureNumber = Obj((n-1)*length(TVec)+t).FailureNumber + 1;
%             end
            
            % Method 5: Reduced number of constraints - greedy search 2
            ...
            if abs(max(demand_M1 - demand_M5)) > 10^(-2) || abs(max(generation_M1 - generation_M5)) > 10^(-2)
                Obj((n-1)*length(TVec)+t).FailureNumber = Obj((n-1)*length(TVec)+t).FailureNumber + 1;
            end
            
        end
        Obj((n-1)*length(TVec)+t).Success_Rate = Success_Rate;
